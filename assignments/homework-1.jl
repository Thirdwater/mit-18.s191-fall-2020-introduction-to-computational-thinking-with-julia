...
function create_bar()
	...
	bar = zeros(numElements)
	center = numElements ÷ 2
	centerOffset = centerCount ÷ 2
	centerRange = (center - centerOffset):(center + centerOffset)
	[bar[i] = 1 for i in centerRange]
	return bar
end
...
function vecvec_to_matrix(vecvec)
	return copy(transpose(hcat(vecvec...)))
end
...
function matrix_to_vecvec(matrix)
	return copy([ vec(matrix[row,:]) for row in 1:size(matrix)[1] ])
end
