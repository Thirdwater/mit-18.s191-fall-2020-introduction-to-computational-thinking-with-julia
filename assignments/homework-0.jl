...
ex_1_1 = md"""
For rigorous proof, see the Babylonian method (as a special case of the more general Newton's method).


Part 1 - square root must be between x/a and a:
For a = sqrt(x), we are fine.
...
Similarly, for a < sqrt(x),
  a * sqrt(x) < x
  so a < sqrt(x) < x / a


Part 2 - vs bisection search
Consider x = 100, a = 50:
In the next iteration, bisection search cuts the space in half to [0, 50].
But in Newton's, the space is smaller at [2, 50].
I.e. [x, x / a] (or [x / a, a]) is a tigher bound than [low, mid] or [mid, high].
